...
class WarrantyController extends Controller
{
    ...
    public function transferToDiscard(Request $request)
    {
        try {
            // Validate tham số
            $validated = $request->validate([
                'warehouse_id' => 'required|integer|exists:warehouses,id',
                'discard_warehouse_id' => 'required|integer|exists:warehouses,id|different:warehouse_id',
                'products' => 'required|array|min:1',
                'products.*.product_id' => 'required|integer|exists:products,id',
                'products.*.quantity' => 'required|integer|min:1',
            ...
            return DB::transaction(function () use ($validated) {
                ...
                $transfer = Transfer::create([
                    'from_warehouse_id' => $validated['warehouse_id'],
                    'to_warehouse_id' => $validated['discard_warehouse_id'],
                    'type' => 'discard',
                    'reason' => $validated['reason'] ?? 'Product cannot be repaired',
                ]);
                ...
                return response()->json([
                    'message' => 'Transfer to discard warehouse successful',
                    'transfer_id' => $transfer->id,
                ], 200);
            });
        } catch (\Illuminate\Validation\ValidationException $e) {
            ...
            return response()->json([
                'error' => 'Failed to transfer to discard warehouse.',
                'message' => $e->getMessage(),
            ], 400);
        }
    }
